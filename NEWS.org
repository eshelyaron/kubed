...
When you press ~l~ in a resource list buffer to show logs for the
resource at point, Kubed now shows only the most recent 128 log lines
by default, to avoid flooding the logs buffer with the full history of
long-running workloads.  You can customize this limit with the new
user option ~kubed-logs-tail-lines~; set it to 0 to restore the
previous behavior of fetching all available log lines.
